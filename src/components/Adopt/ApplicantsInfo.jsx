...
const ApplicantsInfo = () => {
  ...
  const [alternatephonenumber, setAlternatePhoneNumber] = useState('639');
  ...
  const handlePhoneNumberChange = (e) => {
    const input = e.target.value;
    const sanitizedInput = input.replace(/\D/g, ''); // Remove non-numeric characters
    const limitedInput = sanitizedInput.slice(0, 12); // Limit the input to 12 characters
  ...
  const handlePhoneNumberChange1 = (e) => {
    ...
    const sanitizedInput = input.replace(/\D/g, ''); // Remove non-numeric characters
    const limitedInput = sanitizedInput.slice(0, 12); // Limit the input to 12 characters
    setAlternatePhoneNumber(limitedInput);
  };
